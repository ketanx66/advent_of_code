...
bool isRangeOverlaping(RANGE range1, RANGE range2){
    return (isRangeContained(range1,range2) ||
            (range1.right>=range2.left && range2.right>=range1.left));
}
...
int main(){
    ...
    while(fgets(buffer, BUFFER_SIZE, input_fp)){
        ...
        totalPairsContained += isRangeOverlaping(elf1,elf2) ? 1 : 0;
    }
    printf("%u\n",totalPairsContained);
}